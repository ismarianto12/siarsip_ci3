...
class Sppd extends CI_Controller
{
    ...
    public function tambah_data()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $exist_code = $this->db->get_where('sppd', ['letter_code' => $this->input->post('letter_code')])->num_rows();
            if ($exist_code > 0) {
				// http_response_code(500);
                $res = [
                    'response' => 'n',
                    'message' => 'Data no letter surat sudah ada sebelumnya ',
                ]; 
    ...
    public function _rules()
    {
        $this->form_validation->set_rules(
            ...
            'letter_code',
            'trim|required',
            'is_unique[sppd.letter_code]',
            array('letter_code' => 'Sudah dientrikan sebelumnya'),
